...
class LiquidNetWork(
  ...
  def forward(sparseFeats: Map[String, Tensor], sequenceFeats: Map[String, Tensor]): Tensor = {
    // ✅ 唯一一次调用，两个map都传，绝对不报错
    val full = embeddingLayer.forward(sparseFeats, sequenceFeats, squeeze = true)

    // ✅ 序列特征直接从输入拿，不从embedding拿！！！
    val seqTensor = sequenceFeats.head._2
    val batchSize = seqTensor.size(0)
    val seqLen = seqTensor.size(1)

    // ✅ 直接用原始序列张量查嵌入，然后 mean pool
    var seqEmb = embeddingLayer.forward(sequenceFeats)
    // Ensure seqEmb is on the target device
    if (device != "cpu") {
      seqEmb = seqEmb.to(targetDevice, ScalarType.Float)
    }

    // seqEmb: (batch, seq_len, embedDim) → pool to (batch, embedDim)
    val seqEmbPooled = seqEmb.mean(1)

    // Get first sequence item's embedding for ODE initial state
    val firstSeqEmb = seqEmb.select(1, 0)  // (batch, embedDim)
