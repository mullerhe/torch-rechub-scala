...
object RunFraudAntiFraudPipeline {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val maxRows = p.getOrElse("max_rows", "20000").toInt
    val batchSize = p.getOrElse("batch_size", "512").toInt
    ...
    reports += runReport("MEMBA", "ranking-seq") {
      ...
      val trainer = new CTRTrainer(model, learningRate = 1e-3f, device = device, numEpochs = epochs, verbose = false)
    ...
    reports += runReport("MIND", "matching") {
      val (tr, va, te) = buildMatchingSequenceData(prepared, seqEmbedDim = 16)
      val userFeatures = List(SparseFeature("user_id", prepared.userVocab + 2, 16))
      val seqFeature = SequenceFeature("seq_feat", prepared.itemVocab + 2, 16, maxLen = prepared.seqLen)
      val model = new MIND(features = userFeatures, sequenceFeature = seqFeature, embedDim = 16, numInterests = 4, capsuleDim = 8, mlpDims = List(64L, 32L), dropout = 0.1f, device = device)
  ...
  private def runMultiTaskFraud(prepared: FraudPrepared, epochs: Int, batchSize: Int, device: String): Seq[ModelReport] = {
    ...
    def trainMtl(name: String, model: Module): ModelReport = runReport(name, "multitask") {
      val trainer = new MTLTrainer(model, taskNames = List("fraud", "high_amount"), learningRate = 1e-3f, device = device, numEpochs = epochs, verbose = false)
  ...
  private def trainRQVAE(model: RQVAE, train: Vector[EncodedRow], epochs: Int, batchSize: Int, device: String): Unit = {
    ...
    while (e < epochs) {
      ...
      while (i < train.size) {
        ...
        val (totalLoss, _) = model.computeLoss(decoded, qloss, x, "mse")
        totalLoss.backward(); optimizer.step()
        i += bs
      }
      e += 1
    }
