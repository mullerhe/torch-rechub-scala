...
object RunGradNormReplicaBenchmark {
...
//    val modelName = p.getOrElse("model_name", "sharedbottom").toLowerCase
//    val modelName = p.getOrElse("model_name", "sharedbottom").toLowerCase
    val epoch = p.getOrElse("epoch", "8").toInt
    ...
    // Approximate GradNorm by emphasizing the harder conversion task in fixed weights.
    ...
    println(s"[GRADNORM_REPLICA] model=$modelName metrics=$metrics")
