...
object RunSAINTPlusPlusKtBenchmark {
  def main(args: Array[String]): Unit = {
    ...
    println(s"[PASS] SAINTPlusPlus on $dataset -> " + result.metrics.toSeq.sortBy(_._1).map { case (k, v) => f"$k=$v%.4f" }.mkString(", "))
  }
}
