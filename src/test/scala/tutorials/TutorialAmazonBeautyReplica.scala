...
object TutorialAmazonBeautyReplica {
  def main(args: Array[String]): Unit = {
    val csv = if (args.nonEmpty) args(0) else "/home/muller/IdeaProjects/torch-rechub/examples/ranking/data/amazon-beauty/amazon_beauty_sample.csv"
    ...
    println(f"[AMAZON_BEAUTY_REPLICA] auc=$auc%.4f metrics=$m")
    require(auc >= 0.35f, f"AUC too low: $auc%.4f")
  }
}
